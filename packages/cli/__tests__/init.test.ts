...
function swapWorkspaceDeps(pkg: Record<string, unknown>): Record<string, unknown> {
...
describe('workspace:* → latest dependency swap', () => {
  it('replaces workspace:* with latest in dependencies', () => {
    ...
    expect((pkg.dependencies as Record<string, string>)['@ninna-ui/core']).toBe('latest');
    expect((pkg.dependencies as Record<string, string>)['react']).toBe('^19.0.0');
  });
  ...
  it('replaces workspace:* in devDependencies', () => {
    ...
    swapWorkspaceDeps(pkg);
    expect((pkg.devDependencies as Record<string, string>)['@ninna-ui/utils']).toBe('latest');
    expect((pkg.devDependencies as Record<string, string>)['vitest']).toBe('^4.0.0');
  });
  ...
  it('replaces workspace:* in peerDependencies', () => {
    ...
    swapWorkspaceDeps(pkg);
    expect((pkg.peerDependencies as Record<string, string>)['@ninna-ui/react-internal']).toBe('latest');
  });
  ...
  it('leaves non-workspace:* versions unchanged', () => {
    ...
    expect((pkg.dependencies as Record<string, string>)['react']).toBe('^19.0.0');
    expect((pkg.dependencies as Record<string, string>)['typescript']).toBe('workspace:^');
  });
...
describe('CSS preset import swap', () => {
  ...
  it('supports all 4 non-default presets', () => {
    ...
    for (const preset of ['ocean', 'sunset', 'forest', 'minimal']) {
      expect(swapPresetInCss(template, preset)).toContain(`presets/${preset}.css`);
    }
  });
});
...
describe('template directories exist', () => {
  for (const template of ['vite-react', 'nextjs', 'react-router', 'astro']) {
    it(`templates/${template}/package.json exists`, () => {
      ...
      expect(existsSync(pkgPath), `${template}/package.json not found at ${pkgPath}`).toBe(true);
    });
  }
});
