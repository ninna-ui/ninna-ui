...
> **5 accessible React navigation components** - Tabs, Accordion, Breadcrumbs, Pagination, and Stepper with full keyboard navigation, WAI-ARIA compliance, and semantic HTML. Radix-powered where it matters, custom-built where it's better.
...
Navigation patterns for [Ninna UI](../../README.md). Tabs and Accordion are powered by [Radix UI](https://www.radix-ui.com/) internally for bulletproof WAI-ARIA compliance and keyboard navigation. Breadcrumbs, Pagination, and Stepper are custom implementations using semantic HTML (`<nav>`, `<ol>`, `aria-current`) - no Radix needed for these simpler patterns.
