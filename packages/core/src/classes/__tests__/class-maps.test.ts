  ...
  describe.each(Object.entries(ALL_MAPS))('%s', (_name, map) => {
    it('contains only non-empty string values', () => {
      for (const [key, value] of Object.entries(map)) {
        expect(typeof value, `value for "${key}" must be a string`).toBe('string');
        expect((value as string).trim().length, `value for "${key}" is empty`).toBeGreaterThan(0);
      }
    });

    it('contains no dark: prefixed classes (hard architecture rule)', () => {
      for (const [key, value] of Object.entries(map)) {
        expect(value, `"${key}" must not contain dark: classes`).not.toMatch(/(?:^|\s)dark:/);
      }
    });

    it('contains no hardcoded palette colors', () => {
      for (const [key, value] of Object.entries(map)) {
        expect(value, `"${key}" must use semantic tokens, not palette colors`).not.toMatch(
          HARDCODED_PALETTE
        );
      }
    });
    ...
    for (const [name, map] of Object.entries(ALL_MAPS)) {
      for (const [key, value] of Object.entries(map)) {
        ...
        for (const token of tokens) {
          expect(token, `${name}.${key} contains an invalid class token`).toMatch(
            /^[a-z][\w:[\]/.-]*$/i
          );
        }
      }
