...
const ENGINE_SHAPES: Record<
  string,
  { engine: Record<string, unknown>; parts: string[] }
> = {
  AccordionEngine: {
...
describe("engine shapes", () => {
  describe.each(Object.entries(ENGINE_SHAPES))(
    "%s",
    (_name, { engine, parts }) => {
      ...
      it("every part is a renderable React component", () => {
        for (const part of parts) {
          expect(
            isRenderable(engine[part]),
            `part "${part}" is not renderable`,
          ).toBe(true);
        }
      });
    },
...
describe("engine rendering (smoke)", () => {
  ...
  it("renders RadioEngine group with items", () => {
    ...
    expect(
      screen.getByRole("radiogroup", { name: "choices" }),
    ).toBeInTheDocument();
